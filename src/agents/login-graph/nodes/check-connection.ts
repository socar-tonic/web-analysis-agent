...
// JavaScript interceptor to capture full request/response data including bodies
...
export async function checkConnection(
  ...
  try {
    ...
    // Inject network interceptor to capture full request/response data
    try {
      const interceptorResult = await mcpClient.callTool({
        name: 'browser_evaluate',
        arguments: { function: NETWORK_INTERCEPTOR_JS },
      });
      console.log(`  [checkConnection] ${extractTextFromMcpResult(interceptorResult)}`);
    } catch (e) {
      console.log(`  [checkConnection] Failed to inject interceptor: ${(e as Error).message}`);
    }
